...
<xsl:transform 
  ...
  <xsl:variable name="iv.active">
    <complexcat>
      <xsl:copy-of select="$s.1.E.dcl.active.default"/>
      <slash dir="\" mode="&lt;"/>
      <xsl:copy-of select="$np.2.X.nom.default"/>
    </complexcat>
  </xsl:variable>
  ...
  <xsl:variable name="tv.passive">    
    <complexcat>
      <xsl:copy-of select="$s.1.E.dcl.passive.default"/>
      <slash dir="\" mode="&lt;"/>
      <xsl:copy-of select="$np.2.X.nom.default"/>
    </complexcat>
  </xsl:variable>

  <xsl:variable name="tv.active">    
    <xsl:call-template name="extend">
      <xsl:with-param name="elt" select="xalan:nodeset($iv.active)/*"/>
      <xsl:with-param name="ext">
        <slash dir="/" mode="&gt;"/>
        <xsl:copy-of select="$np.3.Y.acc"/>
      </xsl:with-param>
  ...
  <xsl:variable name="cop">    
    <xsl:call-template name="extend">
      <xsl:with-param name="elt" select="xalan:nodeset($iv)/*"/>
      <xsl:with-param name="ext">
        <slash dir="/" mode="&gt;"/>
        <xsl:copy-of select="$np.3.Y.nom"/>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:variable>
  ...
  <xsl:variable name="E.ArgOne.X.ArgTwo.Y">  
    <lf>
      <satop nomvar="E:situation">
        <prop name="[*DEFAULT*]"/>
        <diamond mode="ArgOne"><nomvar name="X:sem-obj"/></diamond>
        <diamond mode="ArgTwo"><nomvar name="Y:sem-obj"/></diamond>
      </satop>
    </lf>
  </xsl:variable>

  <xsl:variable name="E.ArgOne.X">  
    <lf>
      <satop nomvar="E:situation">
        <prop name="[*DEFAULT*]"/>
        <diamond mode="ArgOne"><nomvar name="X:sem-obj"/></diamond>
  ...
  <xsl:variable name="E.ArgOne.X.Passive">  
    <lf>
      <satop nomvar="E:situation">
        <prop name="[*DEFAULT*]"/>
        <diamond mode="ArgOne"><nomvar name="X:sem-obj"/></diamond>
  ...
  <xsl:variable name="E.ArgTwo.X.ArgOne.Y">  
    <lf>
      <satop nomvar="E:situation">
        <prop name="[*DEFAULT*]"/>
        <diamond mode="ArgTwo"><nomvar name="X:sem-obj"/></diamond>
        <diamond mode="ArgOne"><nomvar name="Y:sem-obj"/></diamond>
      </satop>
    </lf>
  </xsl:variable>
  ...
  <family name="TransitiveVerbs" pos="V" closed="true">
    <entry name="Primary">
      <xsl:call-template name="extend">
        <xsl:with-param name="elt" select="xalan:nodeset($tv.active)/*"/>
        <xsl:with-param name="ext" select="xalan:nodeset($E.ArgOne.X.ArgTwo.Y)/*"/>
      </xsl:call-template>
    </entry>
    <entry name="Passive">
      <xsl:call-template name="extend">
        <xsl:with-param name="elt" select="xalan:nodeset($tv.passive)/*"/>
        <xsl:with-param name="ext" select="xalan:nodeset($E.ArgOne.X)/*"/>
      </xsl:call-template>
    </entry>
...
<!--  <family name="TransitiveVerbs" pos="V" closed="true">
    <entry name="Primary">
      <xsl:call-template name="extend">
        <xsl:with-param name="elt" select="xalan:nodeset($tv)/*"/>
        <xsl:with-param name="ext" select="xalan:nodeset($E.ArgTwo.X.ArgOne.Y)/*"/>
      </xsl:call-template>
    </entry>
  </family>-->

<!--  <family closed="true" pos="V" name="TransitiveVerbs">
    <entry name="Primary">
      <complexcat>
        <atomcat type="s">
          <fs id="1">
...
<!--  <family name="Experiencer-Subj" pos="V" closed="true">
  ...
  <xsl:variable name="E.be.ArgOne.X.ArgTwo.Y">
    <lf>
      <satop nomvar="E:state">
        <prop name="be-verb"/>
        <diamond mode="ArgOne">
  ...
  <family name="Copula" pos="V" closed="true">
    ...
    <entry name="NP">
      <xsl:call-template name="extend">
        <xsl:with-param name="elt" select="xalan:nodeset($cop)/*"/>
        <xsl:with-param name="ext" select="$E.be.ArgOne.X.ArgTwo.Y"/>
      </xsl:call-template>
    </entry>
