...
<xsl:transform 
  ...
  <xsl:template match="word">
	...
	<xsl:variable name="all-macros">
	  <xsl:choose>
	    <xsl:when test="../@pos='V' and not(contains($macros, '@active')) and (contains($macros, '@past') or contains($macros, '@pres'))">
	      <xsl:value-of select="normalize-space(concat($macros, ' ', '@active'))"/>
	    </xsl:when>
	    <xsl:otherwise>
	...
	<xsl:attribute name="macros"><xsl:value-of select="$all-macros"/></xsl:attribute>
      </xsl:if>
